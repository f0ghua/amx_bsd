...
(*
    ...
    device address for each command, if desired. Increased MB_MAX_POLL_ITEMS to 500.
    
    This version of the module was tested piecewise, without a real MODBUS device.
*)
(*
...
*)
(*
    Version 1.5 replaces Comm Queue WAIT with timeline to send next commands at 0.5 sec.
    intervals if no valid reply is received
*)
(*
    Version 1.6 04-24-08 SMA - added line to set bool bStringOK = TRUE (line 1432),
    if no other error conditions exist.  This is required to satisfy the conditional
    on Line 1845, which allows the buffer to be cleared and fnDEQ () to empty the command
    buffer.  There was no other location in code in which bStringOK was set to true, so the
    conditional would never be satisfied.
*)
(***********************************************************)
(*}}PS_SOURCE_INFO                                         *)
(***********************************************************)
...
DEFINE_VARIABLE
    ...
    VOLATILE INTEGER nDebug = FALSE                 // DETERMINES IF DEBUGGING IS ON OR OFF  
...
//          (4) IN - bAsciiFlag  indicates whether NetHex should translate ASCII characters or not 
// Returns: none
...
//          sends it to the device. Pauses the timeline if 
//          the queue is empty
...
//  
//    
DEFINE_FUNCTION integer fnAddPollItem(char sData1[], char sData2[], char sData3[], char sData4[], char sData5[])
{
    ...
    {
        if ( (nAddress == uModbusPollList[i].nAddress) AND (nItemType == uModbusPollList[i].nItemType)
		AND (cDevAddr == uModbusPollList[nNumberPollItems].nDevAddress) )
        {
...
DEFINE_FUNCTION fnProcessAPICommands(CHAR sCmdArray[])
{
    ...
    {   
        ...
        *)
		...
		ELSE 
		    SEND_COMMAND dvDEVICE,"'TSET BAUD ',ITOA(lBaudRate),',N,8,1 485 DISABLE'" 
